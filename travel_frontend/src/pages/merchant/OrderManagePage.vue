<template>
  <div class="order-manage-page">
    ...
    <a-empty v-if="!loading && orders.length === 0" description="暂无订单">
      <template #image>
        <FileTextOutlined style="font-size: 48px; color: var(--color-gray-ccc)" />
      </template>
    </a-empty>
    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-else-if="column.key === 'totalAmount'">
          <span style="color: var(--support-500); font-weight: bold">¥{{ (record.totalAmount ?? 0).toFixed(2) }}</span>
        </template>
        <template v-else-if="column.key === 'actualAmount'">
          <span style="color: var(--success-500); font-weight: bold">¥{{ (record.actualAmount ?? 0).toFixed(2) }}</span>
        </template>
    ...
    >
      <a-form :label-col="{ span: 6 }" :wrapper-col="{ span: 18 }">
        ...
        <a-form-item label="物流单号">
          ...
          />
          <div style="margin-top: 4px; color: var(--color-gray-999); font-size: 12px">
...
const getStatusColor = (status?: number) => {
  switch (status) {
    case 0:
      return 'warning'  // 待支付 -> 警告色（日落金）
    case 1:
      return 'processing' // 已支付 -> 处理中（蓝色）
    case 2:
      return 'default'  // 待发货 -> 默认
    case 3:
      return 'success'  // 已发货 -> 成功色（青绿）
    case 4:
      return 'warning'  // 退款中 -> 警告色（日落金）
    case 5:
      return 'error'    // 已退款 -> 错误色（珊瑚红）
    default:
      return 'default'
  }
}
